<template>
	<view class="login-box">
		...
		<view class="btn-login-box">
			<btn type="0" v-model="username"   ></btn>
			<btn type="1" v-model="password"  ></btn>
		</view>
		<view class="remember-box">
			<switch class="switch-box" checked color="#ECB7BB" />
			<view class="remember-text">Remember me</view>
		</view>
		<comm-btn  @click="userLogin"></comm-btn>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods:{
			async userLogin(){
			 let  {code,data} = await this.$api.user.login({phone:this.username,password:this.password });
			 console.log(code)
			 if(code==0){
				 //登录成功 设置用户缓存
